function cumacc=UT_evaluate_per_segments(cumacc,nodesize, classlist, len,nCenter,Threshold)
...
%     end
    fmeasure = (1+0.3^2)*(precision.*recall)./((0.3^2)*precision+recall);
        cumacc= cumacc+fmeasure;
        [mf midx] =max(fmeasure);
    fprintf('f-measure : %.2f /%.2f  ',fmeasure(ceil(Threshold*1000)),max(fmeasure));
    fprintf('max threshold: %d  /%.2f  ',midx,mf);
    fprintf('my acc : %.2f / %.2f',segacc(100), max(segacc));%330
...
%     hold off;
%     print(h,'-dpdf',['./data/UT_pr_curve_img/',lname,'_',num2str(nCenter),'_org.pdf']);
