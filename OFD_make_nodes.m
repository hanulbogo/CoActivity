function OFD_make_nodes(nodesize,classlist,cpath, hpath,NC)
...
for class= classlist
    for i= (class-1)*nVideo+1: nVideo*class%length(ucf_annotation)
        
        fprintf('make node ofd processing %s %d / %d \n',ucf_annotation{i}.name,i,class*nVideo);%length(ucf_annotation));
        load([cpath,ucf_annotation{i}.name,'_ofd.mat']);
        ...
        save([hpath,ucf_annotation{i}.name,'_nodesize_',num2str(nodesize),'_HISTS_ofd.mat'], 'Thists','HOGFhists','HOGhists','OFDLhists','OFDRhists','OFDThists','OFDBhists','OFDAhists','OFDChists','HOFhists','MBHxhists','MBHyhists');
    end
end
