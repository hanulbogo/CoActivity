function OFD_make_segments_ofd(nodesize, classlist, len,NC)
...
for class =classlist% kiss set has the best results
    ...
    for v=1:nVideos
        aidx =(class-1)*nVideos+v;
        load([hpath,ucf_annotation{aidx}.name,'_nodesize_',num2str(nodesize),'_HISTS_ofd.mat']);
        oldhidx = hidx;
    ...
    save([spath,ucf_annotation{aidx}.label,'_nodesize_',num2str(nodesize), '_SEGHISTS_OFD_',num2str(len),'_ofd.mat'],'TSEGHISTS','HOGFSEGHISTS','HOGSEGHISTS','OFDLSEGHISTS','OFDRSEGHISTS','OFDTSEGHISTS','OFDBSEGHISTS','OFDASEGHISTS','OFDCSEGHISTS','HOFSEGHISTS','MBHxSEGHISTS','MBHySEGHISTS');
