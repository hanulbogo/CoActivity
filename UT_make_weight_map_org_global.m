function density_map=UT_make_weight_map_org_global(nodesize, classlist, len,density,nCenter)
...
for class =classlist
    ...
    for v=1:nVideos
        ...
        if maxdensity<max(density_map{v})
            maxdensity=max(density_map{v});
        end
        if mindensity>min(density_map{v})
            mindensity=min(density_map{v});
        end
    end

    for v=1:nVideos
        
        density_map{v}=(density_map{v}-mindensity)/(maxdensity-mindensity);
    end
