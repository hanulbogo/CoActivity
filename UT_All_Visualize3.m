function [precision ,recall,fmeasure,dth] =UT_All_Visualize3( class,Comb)
...
alg_names = {'GT' 'AMC','AMC-','PR','CS','Xiong'};
vistemp = {'GT' 'AMC','AMC-','PR','CS','BC'};
...
hold on;
    set(h, 'Position', [1, 1, 600, 200]);
     set(gcf,'PaperSize',[4 2.3]);
        ...
        set(gcf,'PaperPosition',[-0.1 0 4.2 2.3]);
...
%     end
    ...
    for aidx = 1:nalg
        ...
        line([1.5, v_end(v)], ones(1,2) .* bar_height, 'color', [0.95 0.95 0.95], 'LineWidth', 3);
        for idx = 1:cc.NumObjects
            sttt = cc.PixelIdxList{idx}(1);
            endt = cc.PixelIdxList{idx}(end);
%             if v==1
...
%         end
        ...
        set(gca, 'DataAspectRatio',[105*v_end(v)/1200, 1,2]);
    end
 lname=changelname(lname);
    text(v_end(v)/2,7.2,lname,'FontSize',8,'FontName','Times','HorizontalAlignment','center');
    hold off;
