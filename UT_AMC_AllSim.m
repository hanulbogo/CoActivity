function density=UT_AMC_AllSim(TDATA, HOGDATA,HOFDATA,MBHxDATA,MBHyDATA, class,len,nodesize,nData,nCenter) %data : N*D
dpath = './data/UT_dists/';
if ~exist(dpath,'dir')
    mkdir(dpath);
end
...
for i=1:length(mask)
%     if i> step && i<length(Kern)-step
        tempKern(i,[max(1,i-step*1):i-1,i+1:min(i+step*1,length(mask))])=1;
%     elseif i<=step
...
tempKern=tempKern(1:length(mask),1:length(mask));
tempKern= tempKern+tempKern';
tempKern=tempKern.*(1-mask);
tempKern(tempKern>=1)=1;


Kern1 =KernMask(dists,didx,mask);
...
Kern2 =KernMask(dists,didx,tempKern);

Kern2(nData<len*.5,:)=0;
Kern2(:,nData<len*.5)=0;

Kern2 = Kern2.*tempKern;

Kern = Kern1+Kern2;
x = AMC_AllSim(Kern(nData>=len*.5,nData>=len*.5));
