function [precision ,recall,fmeasure,dth] =UCF_N_plot_detection(density_map, class,Comb,mname)
...
PFpath ='./data/UCF_TWO_POS_Frames/';
if ~exist(PFpath,'dir')
    mkdir(PFpath);
end
...
for v=1:length(Comb)
    ...
    tmp =zeros(v_end(v),1);
    gt_start{v} = UCF_annotation{aidx}.gt_start(1:UCF_annotation{aidx}.gt_n);
    gt_end{v}= UCF_annotation{aidx}.gt_end(1:UCF_annotation{aidx}.gt_n);
...
precision = tp/(precisiondenorm+eps);
recall= tp/(recalldenorm+eps);
...
% print(h,'-dpdf',['./data/UCF_Res_img/',lname,'_',num2str(Comb(1)), num2str(Comb(2)) '.pdf']);
% print(h,'-depsc','-tiff',['./data/UCF_Res_img/',lname,'_',num2str(Comb(1)), num2str(Comb(2)) '.eps']);