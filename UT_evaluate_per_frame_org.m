function UT_evaluate_per_frame_org(nodesize, classlist, len,nCenter)
...
for class =classlist% kiss set has the best results
    ...
    [sorted_recall, sorted_precision] = PR_for_draw(sorted_recall, sorted_precision);
    for t= 2:length(sorted_recall)
    ...
    fprintf(' ap : %f %f ',ap*100,precision(recall_idx(end))*100);
%     close(h);
end