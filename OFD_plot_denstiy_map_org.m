function OFD_plot_denstiy_map_org(nodesize, classlist, len,nCenter,Threshold)
load('./data/OFD_TWO_annotation.mat');%'ucf_annotation'
global mname;
dpath =['./data/OFD_density_map/' mname '/'];
...
for class =classlist% kiss set has the best results
    ...
    hold off;
%     print(h,'-dpdf',['./data/OFD_density_img/',lname,'_',num2str(nCenter),'_org.pdf']);
%     close(h);
