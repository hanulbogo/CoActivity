...
%     
%     v_start(v)=1;
%     v_end(v)= sum(TH_annotation{aidx}.nFrames);
%     tmp =zeros(v_end(v),1);
%     gt_start{v} = TH_annotation{aidx}.gt_start;
%     gt_end{v}= TH_annotation{aidx}.gt_end;
%     
...
%         end
 ...
 lname=changelname(lname);
    text(v_end(v)/2,nalg*stepsize+1,[lname ' ' num2str(Comb(v))],'FontSize',8,'FontName','Times','HorizontalAlignment','center');
    hold off;
% 
%     respath =[pdfpath,lname];
%     resvpath = [respath '_' num2str(Comb(v))];
...
% hold off;
% % print(h,'-dpdf',['./data/TH_TWO_RES_IMG/',lname,'/',num2str(Comb(1)), '_',num2str(Comb(2)),'_',num2str(Comb(v)) '.pdf']);
% %     print(h,'-dpng',['./data/TH_TWO_RES_IMG/',lname,'/',num2str(Comb(1)), '_',num2str(Comb(2)),'_',num2str(Comb(v)) '.png']);
% 
