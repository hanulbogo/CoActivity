function KDE_make_segments_all_vid_with_itv(nodesize, classlist, len,vididx,itv)
load('./data/KDE_ucf50_annotation.mat');%'ucf_annotation'
% cpath ='./data/KDE_concat/';
hpath='./data/KDE_concat_hist/';
spath ='./data/seg_hists_all/';
if ~exist(spath,'dir')
    mkdir(spath);
end
D=4000;
...
for class =classlist% kiss set has the best results
    ...
    for v=vididx
        ...
        for n = itv(vcnt,1): itv(vcnt,2)
            n_start = n;
            n_end = n+(len/nodesize)-1;
            n_end= min(size(Tcumhists,1), n_end);
        ...
        TSEGHISTS=TSEGHISTS(1:hidx-1,:);
        HOGSEGHISTS=HOGSEGHISTS(1:hidx-1,:);
        HOFSEGHISTS=HOFSEGHISTS(1:hidx-1,:);
        MBHxSEGHISTS=MBHxSEGHISTS(1:hidx-1,:);
        MBHySEGHISTS=MBHySEGHISTS(1:hidx-1,:);
        
        if vcnt==1
            save([spath,ucf_annotation{aidx}.label,'_nodesize_',num2str(nodesize), '_SEGHISTS_KDE_',num2str(len),'_',num2str(vididx(1)),'_gt.mat'],'TSEGHISTS','HOGSEGHISTS','HOFSEGHISTS','MBHxSEGHISTS','MBHySEGHISTS');
        else
            save([spath,ucf_annotation{aidx}.label,'_nodesize_',num2str(nodesize), '_SEGHISTS_KDE_',num2str(len),'_',num2str(vididx(2)),'_all.mat'],'TSEGHISTS','HOGSEGHISTS','HOFSEGHISTS','MBHxSEGHISTS','MBHySEGHISTS');
        end
