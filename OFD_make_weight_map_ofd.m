function OFD_make_weight_map_ofd(nodesize, classlist, len,density,nCenter)
...
%         density_map{v}= density_map{v}/max(density_map{v});
        if max( density_map{v})>maxdensity
            maxdensity=max(density_map{v});
        end
%         
    end
% %     fprintf('max density %f\n', maxdensity);
    ...
    save([dpath,ucf_annotation{aidx}.label,'_nodesize_',num2str(nodesize), '_DENSITY_MAP_OFD_',num2str(len),'_',num2str(nCenter),'_ofd.mat'],'density_map');
end
