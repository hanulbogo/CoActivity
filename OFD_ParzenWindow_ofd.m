function density=OFD_ParzenWindow_ofd(class,TDATA, HOGDATA, HOGFDATA, OFDLDATA, OFDRDATA, OFDTDATA, OFDBDATA,OFDADATA, OFDCDATA,HOFDATA, MBHxDATA, MBHyDATA,len,nodesize,nData,nCenter)  %data : N*D
...
for i = 1:length(Kern)
    ...
    Ki =Kern(i,:);
    maski = mask(i,:);
  density(i) =std(Ki(maski==1));
end

density(~isnan(density)) = exp(-((density(~isnan(density))/(mean(density(~isnan(density))))).^(-1)));
% density(~isnan(density)) =density(~isnan(density))/sum(density(~isnan(density)));
