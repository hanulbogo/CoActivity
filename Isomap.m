function [Y, R, E] = Isomap(D, n_fcn, n_size, options,startidx, endidx,itv) 
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         figure;
         hold on;
         for v= 1:length(startidx)
             plot(Y.coords{2}(1,startidx(v):endidx(v)), Y.coords{2}(2,startidx(v):endidx(v)), [colors(v),'o-']);
             ...
             plot(Y.coords{2}(1,itv(v,1):itv(v,2)), Y.coords{2}(2,itv(v,1):itv(v,2)), [colors2(rem(v,3)+1),'o-']);
%              plot(Y.coords{2}(1,itv(v,3):itv(v,4)), Y.coords{2}(2,itv(v,3):itv(v,4)), [colors2(rem(v,5)+1),'o-'],'markersize',10);
