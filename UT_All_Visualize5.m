function [precision ,recall,fmeasure,dth] =UT_All_Visualize5( class,Comb)
...
pdfpath = ['./data/UT_ALL_RES_PLOT3/',lname,'/'];
...
hold on;
    set(h, 'Position', [1, 1, 600, 200]);
     set(gcf,'PaperSize',[15 2.3]);
        
        set(gcf,'PaperPositionMode','Manual');
        set(gcf,'PaperPosition',[-1 0.02 16 2.25]);
...
%         hold off;
        ...
        set(gca, 'YTickLabel', wrev(vis_names) ,'FontSize',7,'FontName','Times-Roman');
        nFrame = v_end(v);
%         Nkey=5;
...
%         set(gca, 'DataAspectRatio',[170*v_end(v)/1200, 1,2]);
 ...
 lname=changelname(lname);
    text(v_end(v)/2,nalg*stepsize+1,[lname num2str(Comb,'%02d')],'FontSize',8,'FontName','Times-Roman','HorizontalAlignment','center');
    hold off;
